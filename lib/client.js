...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// 卡片
			".SKM_cards{grid-template-columns:minmax(0,1fr);align-items:stretch;gap:10px;margin:0;padding:0;list-style:none;display:grid}",
			...
			+ ".SKM_cardRow{width:100%;align-items:center;gap:12px;padding:10px 12px 8px;display:flex}"
			+ ".SKM_cardMain{min-width:0;flex:1;flex-direction:column;gap:4px;display:flex}"
			+ ".SKM_cardSide{flex:none;flex-direction:column;align-items:flex-end;justify-content:center;gap:2px;display:flex}"
			+ ".SKM_cardSide .SKM_status{font-size:11px;line-height:16px;max-width:160px;text-align:right}",
			".SKM_cardHead{width:100%;align-items:center;gap:8px;font:inherit;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border:none;padding:0;display:flex;text-align:left}",
