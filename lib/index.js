...
async function loadState() {
  try {
    ...
    if (parsed !== null && typeof parsed === "object") {
      return {
        enabled: Array.isArray(parsed.enabled) ? new Set(parsed.enabled) : new Set(),
  ...
  return { enabled: new Set(), repos: [] };
}

async function saveState(state) {
  await writeFile(
    stateFilePath(),
    JSON.stringify({ enabled: [...state.enabled].sort(), repos: state.repos }, null, 2),
    "utf8"
  );
}
...
class SkillManagerGateway extends TypertRemoteService {
  ...
  /** 下载仓库归档并扫描可发现技能列表（分支回退 main→master）。 */
  async discoverRepo(owner, name, branch) {
    validateRepoRef(owner, name, branch ?? "");
    const { buffer, branch: resolvedBranch } = await fetchRepoArchive(owner, name, branch ?? "");
