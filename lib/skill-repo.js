...
/** 校验仓库坐标（任何会拼进 github.com URL 的入口）。 */
export function validateRepoRef(owner, name, branch) {
...
/**
 * 流式下载归档（大小上限 + 超时）。返回 Buffer。
 * maxBytes 可注入（测试用小上限；默认 128 MiB）。
 */
export async function downloadArchive(url, { maxBytes = MAX_ARCHIVE_DOWNLOAD_BYTES } = {}) {
  ...
  try {
    ...
    throw new Error("下载失败（DOWNLOAD_FAILED）：" + (err?.name === "AbortError" ? "超时" : err?.message ?? err));
  } finally {
    clearTimeout(timer);
  }
  if (!response.ok) {
    const status = String(response.status);
    throw new Error(`下载失败（DOWNLOAD_FAILED）状态 ${status}`);
  }
  const reader = response.body?.getReader();
  if (reader === undefined) throw new Error("下载失败（DOWNLOAD_FAILED）：响应无内容");
  const chunks = [];
  let total = 0;
  for (;;) {
 ...
 */
export async function fetchRepoArchive(owner, name, branch = "", baseUrl = "https://github.com", options = {}) {
  ...
  for (const candidate of branchCandidates(branch)) {
    ...
    const url = `${baseUrl}/${owner}/${name}/archive/refs/heads/${candidate}.zip`;
    try {
...
function archiveCacheKey(owner, name, branch) {
  return `${owner}__${name}__${branch}.zip`;
}
